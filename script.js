    ...
    filtered.forEach(p => {
        grid.innerHTML += `
            <div class="product-card">
                <img src="${p.img}" alt="${p.name}" onerror="this.src='https://via.placeholder.com/300x200?text=Imagine+Lipsa'">
                <h3>${p.name}</h3>
...
window.onload = () => { 
    if(document.getElementById('productGrid')) displayProducts('all'); 
};
